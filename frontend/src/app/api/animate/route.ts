...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[API /animate] Prompt:", prompt);

    // Path to the Python animation sub-agent
    const agentPath = path.join(process.cwd(), "..", "agent");
    const scriptPath = path.join(agentPath, "animation_subagent.py");
    ...
    console.log("[API /animate] Result:", JSON.stringify(result, null, 2));

    return NextResponse.json(result);
  } catch (error: any) {
    console.error("Error calling animation sub-agent:", error);
    return NextResponse.json(
