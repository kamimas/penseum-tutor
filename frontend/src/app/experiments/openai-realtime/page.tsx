...
/**
 * OpenAI Realtime Test Page
 *
 * Test page for OpenAI Realtime API integration.
 * Mirrors the Gemini and xAI latency test structure.
 */

import { useState, useEffect, useRef, useCallback } from "react";
import { OpenAILiveClient, ConnectionState, ToolCall } from "./lib/OpenAILiveClient";
...
export default function OpenAIRealtimePage() {
  ...
  const clientRef = useRef<OpenAILiveClient | null>(null);
  ...
  const handleToolCall = useCallback(
    (toolCalls: ToolCall[]) => {
      ...
      for (const tc of toolCalls) {
        ...
        // Handle draw and clear_board tools - use silent so OpenAI doesn't react
  ...
  const handleConnect = useCallback(async () => {
    ...
    if (!apiKey) {
      addLog("client", "ERROR: NEXT_PUBLIC_OPENAI_API_KEY not set");
      return;
    }
    ...
    >
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <p style={{ color: "#888", marginBottom: 24 }}>
          WebSocket connection to OpenAI Realtime API (24kHz audio)
        </p>
        ...
        >
          <strong>Multi-Tool Test (OpenAI):</strong>
          <ul style={{ marginTop: 8, paddingLeft: 20 }}>
            ...
            <li>Model: gpt-realtime | Voice: Alloy | Audio: 24kHz PCM</li>
          </ul>
        </div>
