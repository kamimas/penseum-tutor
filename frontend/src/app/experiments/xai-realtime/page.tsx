...
/**
 * xAI Realtime Test Page
 *
 * Test page for xAI Grok Realtime API integration.
 * Mirrors the Gemini Direct experiment structure.
 */

import { useState, useEffect, useRef, useCallback } from "react";
import { XaiLiveClient, ConnectionState, ToolCall } from "./lib/XaiLiveClient";
...
export default function XaiRealtimePage() {
  ...
  const clientRef = useRef<XaiLiveClient | null>(null);
  ...
  const speechStoppedTimeRef = useRef<number>(0);
  ...
  const handleToolCall = useCallback(
    (toolCalls: ToolCall[]) => {
      ...
      // Measure time from speech stopped to tool call
      if (speechStoppedTimeRef.current > 0) {
        const latency = now - speechStoppedTimeRef.current;
        addLog("latency", `Speech Stop -> Tool Call: ${latency.toFixed(0)}ms`);
      }

      for (const tc of toolCalls) {
        addLog("tool", `${tc.name}(${JSON.stringify(tc.args)})`);

        // Handle draw and clear_board tools - use silent so xAI doesn't react
        const responseStart = performance.now();
        const result = "Done.";
        clientRef.current?.sendToolResponse(tc.id, result, true);
  ...
  const handleConnect = useCallback(async () => {
    ...
    if (!apiKey) {
      addLog("client", "ERROR: NEXT_PUBLIC_XAI_API_KEY not set");
      return;
    }
    ...
    // Create xAI client
    const client = new XaiLiveClient({
      apiKey,
      voice: "Ara",
      tools: TOOLS,
    ...
    client.on("sessionCreated", (sessionId) => {
      addLog("server", `Session created: ${sessionId}`);
      addLog("client", "Ready - click 'Test Tool' to measure latency");
    });
    ...
    client.on("speechStarted", () => {
      addLog("server", "Speech detected - interrupting");
    ...
    client.on("speechStopped", () => {
      speechStoppedTimeRef.current = performance.now();
      addLog("server", "Speech stopped");
    });
    ...
    client.on("responseDone", () => {
      // Reset output transcript for next response
      setOutputTranscript("");
    });
  ...
  const handleToggleMic = useCallback(async () => {
    if (isMicEnabled) {
      ...
      try {
        ...
        addLog("client", "Microphone enabled (24kHz)");
      } catch (err) {
  ...
  const handleTestDraw = useCallback(() => {
    ...
    firstAudioTimeRef.current = 0;
    speechStoppedTimeRef.current = 0;
    ...
    >
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        <h1 style={{ fontSize: 24, fontWeight: 600, marginBottom: 8 }}>
          xAI Grok Realtime Test
        </h1>
        <p style={{ color: "#888", marginBottom: 24 }}>
          WebSocket connection to xAI Realtime API (24kHz audio)
        </p>
        ...
        >
          <strong>Multi-Tool Test (xAI):</strong>
          <ul style={{ marginTop: 8, paddingLeft: 20 }}>
            ...
            <li>Voice: Ara | Audio: 24kHz PCM</li>
          </ul>
        </div>
      </div>
    </div>
