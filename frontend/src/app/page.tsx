...
// Dynamic import - Landing Scene (Three.js doesn't support SSR)
const LandingScene = dynamic(
  () => import("../components/landing/InvestorScene"),
  {
    ssr: false,
    loading: () => (
      <div className="fixed inset-0 bg-black flex items-center justify-center">
        <div className="text-center">
          <div className="text-white/30 font-mono text-xs tracking-[0.2em] uppercase animate-pulse">
            Initializing...
          </div>
        </div>
      </div>
...
const SYSTEM_INSTRUCTION = `You are PAI, a tutor by Penseum. Concise.

# RULES
- EVERY turn: call draw() AND talk at the same time (in parallel). Don't wait for draw() to finish.
- If you don't draw(), you failed.
- Every 3-4 turns, use draw("quiz on [topic]") to test understanding (in addition to verbal questions).
- Be truthful. Correct mistakes kindly.

# TOOLS
- draw(query) - visualize anything
- clear_board() - clear the whiteboard. Use when: switching to a new topic, board is cluttered, or student asks to start fresh

# DRAW
Short query (intro, simple concept):
  draw("labeled heart diagram")
  draw("food chain: sun, grass, rabbit, fox")
  draw("graph sin(x)")
  draw("quiz on photosynthesis")

Long query (explain HOW something works):
  draw("blood flowing through the heart, valves opening as blood enters, closing as it pumps out to the body")
  draw("a ball released from height, falling faster due to gravity, bouncing and losing height each time")
  draw("water molecules heated in a pot, moving faster and faster, escaping as steam into the air")
  draw("a pendulum swinging side to side, gradually slowing down as friction steals its energy")
  draw("sunlight hitting a leaf, chloroplasts absorbing the light, converting CO2 and water into glucose and oxygen")

# EXAMPLES

"Teach me about cells"
→ draw("labeled animal cell diagram")
→ "Cells are the building blocks of life! What do you think the nucleus does?"

"How do cells divide?"
→ draw("a cell growing larger, DNA duplicating inside, then the cell pinching in the middle and splitting into two identical cells")
→ "Cells copy themselves through mitosis! Why does DNA need to copy first?"

"What is photosynthesis?"
→ draw("labeled photosynthesis diagram")
→ "Plants make their own food from sunlight! What three things does the plant need?"

"How does photosynthesis actually work?"
→ draw("sunlight hitting a leaf, light absorbed by chloroplasts, water and CO2 molecules combining inside, glucose molecules forming and oxygen bubbling out")
→ "Light energy gets converted to chemical energy! Where does the oxygen come from?"

"Test me"
→ draw("quiz on current topic")
→ "What's your answer?"

"Now teach me about gravity"
→ clear_board()
→ draw("objects falling towards Earth, showing gravitational pull")
→ "New topic! Gravity pulls everything down. What falls faster - a feather or a bowling ball?"
...
const DRAW_TOOL: FunctionDeclaration = {
  name: "draw",
  description: "Draw on whiteboard: text, images, diagrams, annotations. Also use for asking questions (MCQ, fill-in-the-blank, long answer).",
  parameters: {
    type: Type.OBJECT,
    properties: {
      query: {
        type: Type.STRING,
        description: 'What to draw or question to ask (e.g., "show the water cycle", "quiz the student on photosynthesis")',
      },
    },
    required: ["query"],
  },
...
function MinimalToolbar({ excalidrawAPI, isMobile }: { excalidrawAPI: any; isMobile: boolean }) {
  ...
  // Mobile: collapsible toolbar with animation
  if (isMobile) {
    const activeToolData = tools.find(t => t.id === activeTool);

    return (
      <div style={{ position: "fixed", right: 16, bottom: 100, zIndex: 100 }}>
        <AnimatePresence mode="wait">
          {!isExpanded ? (
            // Collapsed: show only active tool icon
            <motion.button
              key="collapsed"
            ...
            // Expanded: show all tools with close button
            <motion.div
              key="expanded"
              ...
              style={{ display: "flex", flexDirection: "column", gap: 4, background: "#FAF9F7", borderRadius: 22, padding: "6px", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", border: "1px solid #E8E4DE" }}
            >
              {tools.map((tool, index) => (
                <motion.button
                  ...
                  title={tool.label}
                  style={{ width: 36, height: 36, borderRadius: 18, border: activeTool === tool.id ? "1px solid #E2DAFB" : "1px solid transparent", background: activeTool === tool.id ? "#F1EDFD" : "transparent", color: activeTool === tool.id ? "#6F47EB" : "#999", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
                >
                  {tool.icon}
                </motion.button>
              ))}
              ...
              <motion.button
                initial={{ opacity: 0, x: 10 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.15, delay: tools.length * 0.03 }}
                onClick={() => setIsExpanded(false)}
                style={{ width: 36, height: 36, borderRadius: 18, border: "1px solid transparent", background: "transparent", color: "#999", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
              >
...
function OnboardingOverlay({ onDismiss, onEnableMic }: { onDismiss: () => void; onEnableMic: () => void }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      >
        {/* Large Title - 34pt iOS style, serif italic */}
        <h1 style={{
        ...
        {/* Secondary instruction - 15pt, medium weight for emphasis */}
        <p style={{
          fontSize: 15,
          fontWeight: 500,
          color: "#3a3a3a",
          ...
          Enable your mic and say
        </p>

        {/* Example prompt - styled, accent color */}
        <p style={{
          fontFamily: "Georgia, 'Times New Roman', serif",
          fontStyle: "italic",
          fontSize: 17,
        ...
        {/* CTA Button - enables mic and dismisses */}
        <motion.button
          onClick={() => {
            onEnableMic();
            onDismiss();
...
function GeminiRoom({ onReset }: { onReset: () => void }) {
  ...
  // Track when Gemini setup is complete (ready to receive messages)
  ...
  // Create image element from math graph animation result
  const handleMathGraphComplete = useCallback((result: AnimatedMathGraphResult) => {
    if (!excalidrawAPIRef.current || !animatedMathGraph) return;
  ...
  // Dedupe draw calls - only allow ONE draw per 3 seconds (regardless of query)
  ...
  const handleDrawToolCall = useCallback(async (query: string, toolCallId: string) => {
    ...
    // Dedupe: skip if ANY draw was called within 3 seconds
    const now = Date.now();
    if (now - lastDrawTimeRef.current < 3000) {
      console.log("[DRAW] Skipping (rate limited):", query);
      return;
    }
    ...
    try {
      ...
      if (data.toolCalls && Array.isArray(data.toolCalls) && excalidrawAPIRef.current) {
        for (const toolCall of data.toolCalls) {
          triggerToolCall(excalidrawAPIRef.current, toolCall.tool, toolCall.params, handleAnimatedAnnotate, handleAnimatedMathGraph);
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      client.on("setupComplete", () => {
        ...
        // Greeting is now triggered when user clicks "Enable Mic"
      });
      ...
      client.on("log", (type, message) => {
        console.log(`[GEMINI:${type}] ${message}`);
      });
  ...
  // Start session - enables mic and triggers greeting
  const handleStartSession = useCallback(async () => {
    ...
    // Send greeting (only once) - wait for setup complete if needed
    if (!greetingSentRef.current && clientRef.current) {
      greetingSentRef.current = true;

      // Check if setup is complete, if not wait (with timeout)
      let retries = 0;
      const sendGreeting = () => {
        const state = clientRef.current?.getState();
        console.log(`[GREETING] Attempt ${retries + 1}: setupComplete=${setupCompleteRef.current}, state=${state}`);

        if (setupCompleteRef.current && state === "connected") {
          console.log("[GREETING] Sending greeting to Gemini...");
          // Ensure audio context is ready for playback
          audioPlaybackRef.current?.resume();
          // Use sendRealtimeText to trigger audio response from native audio model
          clientRef.current?.sendRealtimeText("Session started. Greet the student and ask what they want to learn.");
          console.log("[GREETING] Greeting sent!");
        } else if (retries++ < 50) {
          // Retry up to 5 seconds (50 * 100ms)
          setTimeout(sendGreeting, 100);
        } else {
          console.error("[GREETING] Timeout waiting for Gemini setup");
        }
      };
      sendGreeting();
    }
  ...
  return (
    <div style={{ width: "100vw", height: "100dvh" }}>
      ...
      <AnimatePresence>
        {showOnboarding && (
          <OnboardingOverlay onDismiss={() => setShowOnboarding(false)} onEnableMic={handleStartSession} />
        )}
      </AnimatePresence>
    </div>
  );
  ...
  return (
    <AnimatePresence mode="wait">
      {!started ? (
        ...
        >
          <LandingScene onStart={() => setStarted(true)} />
        </motion.div>
      ) : (
        <motion.div
          key="tutor"
        ...
        >
          <GeminiRoom onReset={() => setStarted(false)} />
        </motion.div>
      )}
    </AnimatePresence>
  );
